rm(list = ls()) 
setwd("~/Documents/Rscripts/shiny/vanduzenapp")
library(tidyverse)
library(lubridate)
...
dtrmnstc$stage <- approx(x = rating$brgc1_cfs_1.6.16, y = rating$brgc1_ft_1.6.16, xout = dtrmnstc$cfs)$y
...
usgs_obs <- read_csv("data/vanduz_obs.csv") %>% mutate(pdt  = ymd_hms(pdt, tz="America/Los_Angeles")) %>%
  mutate(gmt = with_tz(pdt, "Europe/London"), cfs = as.numeric(cfs), kcfs = cfs/1000, stage_ft = as.numeric(stage_ft))

usgs_obs_hrly <- usgs_obs %>% filter(minute(pdt) == 0 )


csv13jan16.12z_hefs <- read_csv("data/2016011312_brgc1.csv") %>% mutate(fcast_t_gmt = mdy_hm(GMT)) %>% select(-"GMT")  %>% mutate(fcast_i_pdt = fcasts[1], fcast_i_pdt_date = date(fcast_i_pdt) ) %>% filter(fcast_t_gmt <= "2016-01-19 08:00")
csv14jan16.12z_hefs <- read_csv("data/2016011412_brgc1.csv") %>% mutate(fcast_t_gmt = mdy_hm(GMT)) %>% select(-"GMT")  %>% mutate(fcast_i_pdt = fcasts[4], fcast_i_pdt_date = date(fcast_i_pdt) ) %>% filter(fcast_t_gmt <= "2016-01-19 08:00")
csv15jan16.12z_hefs <- read_csv("data/2016011512_brgc1.csv") %>% mutate(fcast_t_gmt = mdy_hm(GMT)) %>% select(-"GMT")  %>% mutate(fcast_i_pdt = fcasts[7], fcast_i_pdt_date = date(fcast_i_pdt) ) %>% filter(fcast_t_gmt <= "2016-01-19 08:00")
csv16jan16.12z_hefs <- read_csv("data/2016011612_brgc1.csv") %>% mutate(fcast_t_gmt = mdy_hm(GMT)) %>% select(-"GMT")  %>% mutate(fcast_i_pdt = fcasts[10],fcast_i_pdt_date = date(fcast_i_pdt) ) %>% filter(fcast_t_gmt <= "2016-01-19 08:00")
csv17jan16.12z_hefs <- read_csv("data/2016011712_brgc1.csv") %>% mutate(fcast_t_gmt = mdy_hm(GMT)) %>% select(-"GMT")  %>% mutate(fcast_i_pdt = fcasts[12],fcast_i_pdt_date = date(fcast_i_pdt) ) %>% filter(fcast_t_gmt <= "2016-01-19 08:00")
...
csv13jan16.12z_hefs$stage <- approx(x = rating$brgc1_cfs_1.6.16, y = rating$brgc1_ft_1.6.16, xout = csv13jan16.12z_hefs$cfs)$y
csv14jan16.12z_hefs$stage <- approx(x = rating$brgc1_cfs_1.6.16, y = rating$brgc1_ft_1.6.16, xout = csv14jan16.12z_hefs$cfs)$y 
csv15jan16.12z_hefs$stage <- approx(x = rating$brgc1_cfs_1.6.16, y = rating$brgc1_ft_1.6.16, xout = csv15jan16.12z_hefs$cfs)$y 
csv16jan16.12z_hefs$stage <- approx(x = rating$brgc1_cfs_1.6.16, y = rating$brgc1_ft_1.6.16, xout = csv16jan16.12z_hefs$cfs)$y 
csv17jan16.12z_hefs$stage <- approx(x = rating$brgc1_cfs_1.6.16, y = rating$brgc1_ft_1.6.16, xout = csv17jan16.12z_hefs$cfs)$y 
...
#df_hefs$stage <- approx(x = rating$brgc1_cfs_1.6.16, y = rating$brgc1_ft_1.6.16,
 #                       xout = df_hefs$cfs)$y
...
#### type 4 quants
#                              create_hrly_quants: f(df, hourcolumn, value, qtype, metadata_to_add) 
csv13jan16.12z_hefs_s_qntls_h_t4 <- create_hrly_quants(csv13jan16.12z_hefs, "fcast_t_gmt", "stage", 4, fcasts[1]) 
csv14jan16.12z_hefs_s_qntls_h_t4 <- create_hrly_quants(csv14jan16.12z_hefs, "fcast_t_gmt", "stage", 4, fcasts[4]) 
csv15jan16.12z_hefs_s_qntls_h_t4 <- create_hrly_quants(csv15jan16.12z_hefs, "fcast_t_gmt", "stage", 4, fcasts[7]) 
csv16jan16.12z_hefs_s_qntls_h_t4 <- create_hrly_quants(csv16jan16.12z_hefs, "fcast_t_gmt", "stage", 4, fcasts[10]) 
csv17jan16.12z_hefs_s_qntls_h_t4 <- create_hrly_quants(csv17jan16.12z_hefs, "fcast_t_gmt", "stage", 4, fcasts[12])


#### type 5 quants
#                              create_hrly_quants: f(df, hourcolumn, value, qtype, metadata_to_add) 
csv13jan16.12z_hefs_s_qntls_h_t5 <- create_hrly_quants(csv13jan16.12z_hefs, "fcast_t_gmt", "stage", 5, fcasts[1]) 
csv14jan16.12z_hefs_s_qntls_h_t5 <- create_hrly_quants(csv14jan16.12z_hefs, "fcast_t_gmt", "stage", 5, fcasts[4]) 
csv15jan16.12z_hefs_s_qntls_h_t5 <- create_hrly_quants(csv15jan16.12z_hefs, "fcast_t_gmt", "stage", 5, fcasts[7]) 
csv16jan16.12z_hefs_s_qntls_h_t5 <- create_hrly_quants(csv16jan16.12z_hefs, "fcast_t_gmt", "stage", 5, fcasts[10]) 
csv17jan16.12z_hefs_s_qntls_h_t5 <- create_hrly_quants(csv17jan16.12z_hefs, "fcast_t_gmt", "stage", 5, fcasts[12]) 

#### type 6 quants
#                              create_hrly_quants: f(df, hourcolumn, value, qtype, metadata_to_add) 
csv13jan16.12z_hefs_s_qntls_h_t6 <- create_hrly_quants(csv13jan16.12z_hefs, "fcast_t_gmt", "stage", 6, fcasts[1]) 
csv14jan16.12z_hefs_s_qntls_h_t6 <- create_hrly_quants(csv14jan16.12z_hefs, "fcast_t_gmt", "stage", 6, fcasts[4]) 
csv15jan16.12z_hefs_s_qntls_h_t6 <- create_hrly_quants(csv15jan16.12z_hefs, "fcast_t_gmt", "stage", 6, fcasts[7]) 
csv16jan16.12z_hefs_s_qntls_h_t6 <- create_hrly_quants(csv16jan16.12z_hefs, "fcast_t_gmt", "stage", 6, fcasts[10]) 
csv17jan16.12z_hefs_s_qntls_h_t6 <- create_hrly_quants(csv17jan16.12z_hefs, "fcast_t_gmt", "stage", 6, fcasts[12]) 

#### type 7 quants
#                              create_hrly_quants: f(df, hourcolumn, value, qtype, metadata_to_add) 
csv13jan16.12z_hefs_s_qntls_h_t7 <- create_hrly_quants(csv13jan16.12z_hefs, "fcast_t_gmt", "stage", 7, fcasts[1]) 
csv14jan16.12z_hefs_s_qntls_h_t7 <- create_hrly_quants(csv14jan16.12z_hefs, "fcast_t_gmt", "stage", 7, fcasts[4]) 
csv15jan16.12z_hefs_s_qntls_h_t7 <- create_hrly_quants(csv15jan16.12z_hefs, "fcast_t_gmt", "stage", 7, fcasts[7]) 
csv16jan16.12z_hefs_s_qntls_h_t7 <- create_hrly_quants(csv16jan16.12z_hefs, "fcast_t_gmt", "stage", 7, fcasts[10]) 
csv17jan16.12z_hefs_s_qntls_h_t7 <- create_hrly_quants(csv17jan16.12z_hefs, "fcast_t_gmt", "stage", 7, fcasts[12]) 

#### type 8 quants
#                              create_hrly_quants: f(df, hourcolumn, value, qtype, metadata_to_add) 
csv13jan16.12z_hefs_s_qntls_h_t8 <- create_hrly_quants(csv13jan16.12z_hefs, "fcast_t_gmt", "stage", 8, fcasts[1]) 
csv14jan16.12z_hefs_s_qntls_h_t8 <- create_hrly_quants(csv14jan16.12z_hefs, "fcast_t_gmt", "stage", 8, fcasts[4]) 
csv15jan16.12z_hefs_s_qntls_h_t8 <- create_hrly_quants(csv15jan16.12z_hefs, "fcast_t_gmt", "stage", 8, fcasts[7]) 
csv16jan16.12z_hefs_s_qntls_h_t8 <- create_hrly_quants(csv16jan16.12z_hefs, "fcast_t_gmt", "stage", 8, fcasts[10]) 
csv17jan16.12z_hefs_s_qntls_h_t8 <- create_hrly_quants(csv17jan16.12z_hefs, "fcast_t_gmt", "stage", 8, fcasts[12]) 

#### type 9 quants
#                              create_hrly_quants: f(df, hourcolumn, value, qtype, metadata_to_add) 
csv13jan16.12z_hefs_s_qntls_h_t9 <- create_hrly_quants(csv13jan16.12z_hefs, "fcast_t_gmt", "stage", 9, fcasts[1]) 
csv14jan16.12z_hefs_s_qntls_h_t9 <- create_hrly_quants(csv14jan16.12z_hefs, "fcast_t_gmt", "stage", 9, fcasts[4]) 
csv15jan16.12z_hefs_s_qntls_h_t9 <- create_hrly_quants(csv15jan16.12z_hefs, "fcast_t_gmt", "stage", 9, fcasts[7]) 
csv16jan16.12z_hefs_s_qntls_h_t9 <- create_hrly_quants(csv16jan16.12z_hefs, "fcast_t_gmt", "stage", 9, fcasts[10]) 
csv17jan16.12z_hefs_s_qntls_h_t9 <- create_hrly_quants(csv17jan16.12z_hefs, "fcast_t_gmt", "stage", 9, fcasts[12]) 
...
#### type 4 quants
##     create_5dyfc_quants <- function(df, modelrun, value, qtype, fcast_i_pdt)   {
#csv13jan16.12z_hefs_s_qntls_fc_t4 <- create_5dyfc_quants(csv13jan16.12z_hefs, "mefp_yr", "stage", 4, fcasts[1]) 
#csv14jan16.12z_hefs_s_qntls_fc_t4 <- create_5dyfc_quants(csv14jan16.12z_hefs, "mefp_yr", "stage", 4, fcasts[4]) 
#csv15jan16.12z_hefs_s_qntls_fc_t4 <- create_5dyfc_quants(csv15jan16.12z_hefs, "mefp_yr", "stage", 4, fcasts[7]) 
#csv16jan16.12z_hefs_s_qntls_fc_t4 <- create_5dyfc_quants(csv16jan16.12z_hefs, "mefp_yr", "stage", 4, fcasts[10]) 
#csv17jan16.12z_hefs_s_qntls_fc_t4 <- create_5dyfc_quants(csv17jan16.12z_hefs, "mefp_yr", "stage", 4, fcasts[12])
#
##### type 5 quants
###     create_5dyfc_quants <- function(df, modelrun, value, qtype, fcast_i_pdt)   {
#csv13jan16.12z_hefs_s_qntls_fc_t5 <- create_5dyfc_quants(csv13jan16.12z_hefs, "mefp_yr", "stage", 5, fcasts[1]) 
#csv14jan16.12z_hefs_s_qntls_fc_t5 <- create_5dyfc_quants(csv14jan16.12z_hefs, "mefp_yr", "stage", 5, fcasts[4]) 
#csv15jan16.12z_hefs_s_qntls_fc_t5 <- create_5dyfc_quants(csv15jan16.12z_hefs, "mefp_yr", "stage", 5, fcasts[7]) 
#csv16jan16.12z_hefs_s_qntls_fc_t5 <- create_5dyfc_quants(csv16jan16.12z_hefs, "mefp_yr", "stage", 5, fcasts[10]) 
#csv17jan16.12z_hefs_s_qntls_fc_t5 <- create_5dyfc_quants(csv17jan16.12z_hefs, "mefp_yr", "stage", 5, fcasts[12])
#
##### type 6 quants
###     create_5dyfc_quants <- function(df, modelrun, value, qtype, fcast_i_pdt)   {
#csv13jan16.12z_hefs_s_qntls_fc_t6 <- create_5dyfc_quants(csv13jan16.12z_hefs, "mefp_yr", "stage", 6, fcasts[1]) 
#csv14jan16.12z_hefs_s_qntls_fc_t6 <- create_5dyfc_quants(csv14jan16.12z_hefs, "mefp_yr", "stage", 6, fcasts[4]) 
#csv15jan16.12z_hefs_s_qntls_fc_t6 <- create_5dyfc_quants(csv15jan16.12z_hefs, "mefp_yr", "stage", 6, fcasts[7]) 
#csv16jan16.12z_hefs_s_qntls_fc_t6 <- create_5dyfc_quants(csv16jan16.12z_hefs, "mefp_yr", "stage", 6, fcasts[10]) 
#csv17jan16.12z_hefs_s_qntls_fc_t6 <- create_5dyfc_quants(csv17jan16.12z_hefs, "mefp_yr", "stage", 6, fcasts[12])
#
##### type 7 quants
###     create_5dyfc_quants <- function(df, modelrun, value, qtype, fcast_i_pdt)   {
#csv13jan16.12z_hefs_s_qntls_fc_t7 <- create_5dyfc_quants(csv13jan16.12z_hefs, "mefp_yr", "stage", 7, fcasts[1]) 
#csv14jan16.12z_hefs_s_qntls_fc_t7 <- create_5dyfc_quants(csv14jan16.12z_hefs, "mefp_yr", "stage", 7, fcasts[4]) 
#csv15jan16.12z_hefs_s_qntls_fc_t7 <- create_5dyfc_quants(csv15jan16.12z_hefs, "mefp_yr", "stage", 7, fcasts[7]) 
#csv16jan16.12z_hefs_s_qntls_fc_t7 <- create_5dyfc_quants(csv16jan16.12z_hefs, "mefp_yr", "stage", 7, fcasts[10]) 
#csv17jan16.12z_hefs_s_qntls_fc_t7 <- create_5dyfc_quants(csv17jan16.12z_hefs, "mefp_yr", "stage", 7, fcasts[12])
#
##### type 8 quants
###     create_5dyfc_quants <- function(df, modelrun, value, qtype, fcast_i_pdt)   {
#csv13jan16.12z_hefs_s_qntls_fc_t8 <- create_5dyfc_quants(csv13jan16.12z_hefs, "mefp_yr", "stage", 8, fcasts[1]) 
#csv14jan16.12z_hefs_s_qntls_fc_t8 <- create_5dyfc_quants(csv14jan16.12z_hefs, "mefp_yr", "stage", 8, fcasts[4]) 
#csv15jan16.12z_hefs_s_qntls_fc_t8 <- create_5dyfc_quants(csv15jan16.12z_hefs, "mefp_yr", "stage", 8, fcasts[7]) 
#csv16jan16.12z_hefs_s_qntls_fc_t8 <- create_5dyfc_quants(csv16jan16.12z_hefs, "mefp_yr", "stage", 8, fcasts[10]) 
#csv17jan16.12z_hefs_s_qntls_fc_t8 <- create_5dyfc_quants(csv17jan16.12z_hefs, "mefp_yr", "stage", 8, fcasts[12])
#
##### type 9 quants
###     create_5dyfc_quants <- function(df, modelrun, value, qtype, fcast_i_pdt)   {
#csv13jan16.12z_hefs_s_qntls_fc_t9 <- create_5dyfc_quants(csv13jan16.12z_hefs, "mefp_yr", "stage", 9, fcasts[1]) 
#csv14jan16.12z_hefs_s_qntls_fc_t9 <- create_5dyfc_quants(csv14jan16.12z_hefs, "mefp_yr", "stage", 9, fcasts[4]) 
#csv15jan16.12z_hefs_s_qntls_fc_t9 <- create_5dyfc_quants(csv15jan16.12z_hefs, "mefp_yr", "stage", 9, fcasts[7]) 
#csv16jan16.12z_hefs_s_qntls_fc_t9 <- create_5dyfc_quants(csv16jan16.12z_hefs, "mefp_yr", "stage", 9, fcasts[10]) 
#csv17jan16.12z_hefs_s_qntls_fc_t9 <- create_5dyfc_quants(csv17jan16.12z_hefs, "mefp_yr", "stage", 9, fcasts[12])
#
